model.lik <- function(theta, t, Y, model = 0, output=1) {
  ...
  # different model structures. < 3, neutral; >= 3, selection
  # model % 3 == 0, (i)+(ii)+(v); 1 (ii); 2 (i)+(v)
  if(model < 3) {
    mu = 1/2+(t-t)
    if(model == 1) { v0 = 0 }
    if(model == 2) { nuf = 0 }
  ...
  # if requested, just output model variance or mean
  if(output == 4)
  {
    return(sigma*sigma)
  }
...
# compare models with observed V'(h) summary statistics
vprime.tab = read.table("Data/le-age-vprime.csv")

# model predictions for V'(h)
